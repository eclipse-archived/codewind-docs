...
**Local scenario**
Codewind works with fully qualified Appsody stacks only. The `stack:` property in the project's `.appsody-config.yaml` file needs to be in the following format: `hostname[:port]/username/reponame[:tag]`. Also, before you work with the stack, on the local system, enter `docker login` to the private registry.
