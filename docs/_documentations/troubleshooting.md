...
-->
## Che sometimes fails to re-install successfully on Red Hat OpenShift on IBM Cloud
Sometimes when Che is re-installed on Red Hat OpenShift on IBM Cloud, the install may fail with the following error:
```
...
This is due to permissions issues in the namespace.

To resolve this, install Che into a namespace other than `che` with the `--chenamespace` flag when running `chectl server:start`.
