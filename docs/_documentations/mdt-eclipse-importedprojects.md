...
duration: 1 minute
permalink: mdteclipseimportedprojects
type: document
...
The Dockerfile is optional, and it describes what development image is used if the development image is different from the production image. If `Dockerfile-lang` does not exist, the Dockerfile is used for the development image instead.
