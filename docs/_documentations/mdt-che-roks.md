---
layout: docs
title: Setting up OpenShift on IKS for Codewind
description: How to set up OpenShift on IKS for Codewind
keywords: codewind, openshift, iks, kubernetes, clusters, cluster, node, nodes, vps, settingup
duration: 1 minute
permalink: mdt-che-roks
type: document
---

# Setting up OpenShift on IKS for Codewind

## Prerequisites
- Activate a paid IBM Cloud account. Trial accounts cannot create OpenShift clusters.
- Start a running OpenShift on IKS cluster with a minimum of 1 node, 4 CPU, and 16 GB RAM.
...
## Setting up NFS on OpenShift on IKS
